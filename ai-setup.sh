...
say "1b/4  Checking there's room on disk"
FREE=$(df -m / | awk 'NR==2{print $4}')
echo "    free disk: ${FREE} MB"
if [ "$FREE" -lt 4000 ]; then
  echo "    Clearing old Docker layers first..."
  docker system prune -a -f >/dev/null 2>&1 || true
  FREE=$(df -m / | awk 'NR==2{print $4}')
  echo "    free after cleanup: ${FREE} MB"
fi
if [ "$FREE" -lt 3000 ]; then
  cat <<DISK

    Not enough disk. The Ollama image plus a small model needs about 3 GB.
    On the Proxmox host, grow this container (no reboot needed):
